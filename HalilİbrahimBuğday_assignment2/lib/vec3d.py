...
class Vec3d:
    ...
    def point(*args: float) -> 'Vec3d':
        if len(args) != 3:
            raise ValueError("Invalid number of arguments for point. Expected 3, got {len(args)}.")
        return Vec3d(*args, 1)

    @staticmethod
    def vector(*args: float) -> 'Vec3d':
        if len(args) != 3:
            raise ValueError("Invalid number of arguments for vector. Expected 3, got {len(args)}.")
        return Vec3d(*args, 0)
    ...
    def dot(self, vec2: 'Vec3d') -> float:
        if not isinstance(vec2, Vec3d):
            raise TypeError(f"Invalid operand type. Expected Vec3d, got {type(vec2)}.")
